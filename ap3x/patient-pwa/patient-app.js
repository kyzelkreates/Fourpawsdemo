/* ═══════════════════════════════════════════════════════
   ...
   Rebuilt: portal home + slide-up screens, one focus each
════════════════════════════════════════════════════════ */
'use strict';
...
const ENRICHMENT = [
  ...
  {id:'scatter', cat:'scent',      icon:'🌿', name:'Scatter Feeding',        desc:'Scatter kibble in grass — activates nose instinct',  time:'5 min',  level:'Beginner',     tip:'Use meal kibble. Slows eating, reduces stress.', phases:[{l:'Scatter kibble in grass',d:10,c:'hold'},{l:'Let them search!',d:60,c:'inhale'},{l:'Jackpot on last piece',d:5,c:'exhale'}], cycles:2},
...
(function seed(){
  ...
  save('fp_sessions',[
    {date:now-D*13,score:5,skills:['Recall','Sit / Down'],mood:['Easily distracted'],duration:8,note:'First session — very excited!'},
    {date:now-D*12,score:4,skills:['Sit / Down'],mood:['Easily distracted'],duration:6,note:'Sit getting more reliable'},
    {date:now-D*11,score:6,skills:['Recall','Focus'],mood:['Over-excited'],duration:10,note:'Recall in garden — 60% success'},
    ...
    {date:now-D*7, score:5,skills:['Loose lead'],mood:['Easily distracted'],duration:8,note:'Tricky near other dogs'},
...
function applyTheme(){
  ...
  const b=$('theme-toggle'); if(b) b.textContent = isDark?'☀️':'🌙';
}
function toggleTheme(){ isDark=!isDark; save('fp_theme',isDark); applyTheme(); }

function updateXP(){
  const c=$('xp-chip'); if(c) c.textContent=`⚡ ${APP.xp} XP`;
}
...
function showToast(msg){
  const old=$('fp-toast'); if(old) old.remove();
  const t=document.createElement('div');
  t.id='fp-toast'; t.className='fp-toast'; t.textContent=msg;
  document.body.appendChild(t);
  clearTimeout(_tt); _tt=setTimeout(()=>{ if(t.parentNode) t.remove(); }, 3000);
}

// ══════════════════════════════════════════════════════
//  SCREEN SYSTEM  — open / close with slide animation
// ══════════════════════════════════════════════════════
...
function openScreen(name) {
  ...
  const renders = {
    ...
    enrichment: ()=>renderEnrichment('all'),
    coach:      initCoach,
    today:      renderToday,
    log:        ()=>{ /* form is static */ },
  };
  if (renders[name]) renders[name]();
  // Trigger chart renders after DOM paint
...
window.addEventListener('popstate', ()=>{
  if (_currentScreen) closeScreen(_currentScreen);
});
...
function initLogForm() {
  ...
  // Tags — skills
  qsa('#skills-tags .tag').forEach(t=>t.addEventListener('click',()=>{
    t.classList.toggle('active');
    const v=t.getAttribute('data-v');
    const i=_skills.indexOf(v); i===-1?_skills.push(v):_skills.splice(i,1);
  }));
  // Tags — mood
  qsa('#mood-tags .tag').forEach(t=>t.addEventListener('click',()=>{
    t.classList.toggle('active');
    const v=t.getAttribute('data-v');
    const i=_mood.indexOf(v); i===-1?_mood.push(v):_mood.splice(i,1);
  }));
...
function submitLog(){
  ...
  const msgs={
    low:'Brilliant session! 🌟 Short, consistent sessions like this build the best foundation.',
    mid:'Good effort! Some challenges, but great persistence. Follow up with an enrichment activity.',
    high:'Tough session — completely normal. Every dog has off days. Always end on a win.',
  };
  const tips={
  ...
  badge.textContent=txts[lvl]; badge.style.background=bgs[lvl]; badge.style.color=cols[lvl];
  $('fb-msg').textContent=msgs[lvl];
  $('fb-tip').textContent=tips[lvl];

  APP.addSession({
    date:Date.now(), score:s,
    skills:[..._skills], mood:[..._mood],
    duration:parseInt($('duration-input').value)||10,
    note:($('session-note').value||'').trim(),
  });
  save('fp_xp', APP.xp+50);
  save('fp_streak', APP.streak+1);
  updateXP(); updateStreak();

  $('log-form-wrap').style.display='none';
  $('log-feedback').style.display='block';
...
function resetLog(){
  ...
  $('log-form-wrap').style.display='block';
  $('log-feedback').style.display='none';
}
...
function renderCourses(){
  ...
  CURRICULUM.forEach(mod=>{
    ...
    const allDone=doneCnt===mod.lessons.length;
    const isActive=mod.id===cur.id;
    ...
    card.innerHTML=`
      ...
      <div class="mod-lessons">
        ${mod.lessons.map((l,li)=>{
          const done=cl.includes(l.id);
          const isNext=!done && li===doneCnt && !isLocked;
          return `
            <div class="lesson-item" onclick="toggleLesson('${l.id}',${isLocked})">
...
function toggleLesson(id, locked){
  ...
  if(done){
    APP.removeLesson(id);
    const l=CURRICULUM.flatMap(m=>m.lessons).find(x=>x.id===id);
    ...
    if(l){
      ...
      if(mod && mod.lessons.every(x=>APP.completedLessons.includes(x.id))){
        setTimeout(()=>showToast(`🏅 Module ${mod.id} done! +${mod.xp} bonus XP 🎉`),1200);
...
function getMilestones(){
  ...
  return [
    {icon:'🎉',label:'First session logged',                unlocked:s.length>=1,   xp:50 },
    ...
    {icon:'📣',label:'Recall practised',                    unlocked:s.some(x=>(x.skills||[]).includes('Recall')), xp:100},
    {icon:'⏸',label:'Stay skill practised',                unlocked:s.some(x=>(x.skills||[]).includes('Stay')), xp:100},
    {icon:'🎓',label:'Module 1 complete',                   unlocked:CURRICULUM[0].lessons.every(l=>APP.completedLessons.includes(l.id)), xp:500},
    {icon:'🏅',label:'Module 2 complete',                   unlocked:CURRICULUM[1].lessons.every(l=>APP.completedLessons.includes(l.id)), xp:600},
...
function renderProgress(){
  const sessions=APP.sessions, total=sessions.length;
  const prog=getLessonProgress();
  ...
  // History
  if(!sessions.length){
    $('history-list').innerHTML='<p class="empty-msg">No sessions yet — log your first one! 🐾</p>'; return;
  }
...
let _progressChartType='score';
function switchProgressChart(type, btn){
  _progressChartType=type;
  qsa('#screen-progress .mini-tab').forEach(b=>b.classList.remove('active'));
...
function renderProgressChart(type){
  ...
  const color=type==='skills'?'#f4a435':'#3a7d44';
  const fill=type==='skills'?'rgba(244,164,53,.13)':'rgba(58,125,68,.13)';
  drawChart(c,labels,data,color,fill,120);
}
...
// ══════════════════════════════════════════════════════
let _enrichCat='all';
function filterEnrich(cat, btn){
  _enrichCat=cat;
  qsa('#enrich-filters .filter-btn').forEach(b=>b.classList.remove('active'));
  if(btn) btn.classList.add('active');
...
function renderEnrichment(cat){
  ...
  list.forEach(e=>{
    ...
    d.className=`enrich-item${done?' done':''}`;
    d.innerHTML=`
      <div class="ei-icon">${e.icon}</div>
      <div class="ei-info">
        <div class="ei-name">${e.name}</div>
        <div class="ei-desc">${e.desc}</div>
        <div class="ei-meta">
          <span class="ei-pill">⏱ ${e.time}</span>
          <span class="ei-pill">${e.level}</span>
        </div>
        <div class="ei-tip">${e.tip}</div>
      </div>
      <div class="ei-side">
        ${done?`<div class="ei-done-badge">✅ Done</div>`:`<button class="ei-start" onclick="startActivity('${e.id}')">Start</button>`}
      </div>`;
...
function startActivity(id){
  const act=ENRICHMENT.find(e=>e.id===id); if(!act) return;
  if(act.instant){ showToast(act.instant); markActivityDone(id); return; }
  runActivity(act);
}
function runActivity(act){
  $('runner-title').textContent=act.name;
  $('activity-runner').style.display='flex';
  let ci=0, pi=0, tmr=null;
  const max=act.cycles||3;
  function next(){
    if(ci>=max){ finishActivity(act.id); return; }
    const ph=act.phases[pi%act.phases.length];
    $('runner-circle').className='runner-circle '+ph.c;
    $('runner-instruction').textContent=ph.l;
    $('runner-progress-txt').textContent=`Round ${ci+1} of ${max}`;
    let t=ph.d; $('runner-timer').textContent=t;
    clearInterval(tmr);
    tmr=setInterval(()=>{ t--; $('runner-timer').textContent=t; if(t<=0){ clearInterval(tmr); pi++; if(pi%act.phases.length===0) ci++; next(); }},1000);
  }
  next();
...
function initCoach(){
  ...
  _coachInited=true;
  addMsg('bot',`Hi there! 👋 I'm your Four Paws AI Training Coach. Ask me anything about training, behaviour or what to try next.`);
  $('coach-quick').innerHTML=QUICK_TOPICS.slice(0,5).map(q=>
    `<button class="coach-quick-btn" onclick="askCoach(${JSON.stringify(q)})">⚡ ${q}</button>`).join('');
...
function sendCoach(){
  const inp=$('coach-input'); const q=inp.value.trim(); if(!q) return;
  addMsg('user',q); inp.value='';
  setTimeout(()=>{
    const lq=q.toLowerCase();
    let resp='Keep sessions under 5 minutes, always end on success, and stay consistent. Log the session with notes so your trainer can see exactly what happened.';
...
function toggleTodayTask(i){
  const tasks=APP.dailyTasks; tasks[i]=!tasks[i]; APP.saveTask(i,tasks[i]);
  if(tasks[i]){ save('fp_xp',APP.xp+10); updateXP(); showToast('✅ Task done! +10 XP'); }
  renderToday();
}
